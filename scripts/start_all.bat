...
echo [1/4] Checking ClawBot Gateway ...
...
REM --- Step 2: Start ClawBot Bridge in background (OpenClaw WS -> OpenAI REST) ---
echo.
echo [2/4] Starting ClawBot Bridge (port 5001) ...
start "ClawBot Bridge" cmd /c "%ROOT%\scripts\start_bridge.bat"
timeout /t 3 /nobreak >nul

REM --- Step 3: Start VITS TTS server in background ---
echo.
echo [3/4] Starting VITS TTS server (port 8020) ...
start "Paimon VITS" cmd /c "%ROOT%\scripts\start_vits.bat"
timeout /t 5 /nobreak >nul

REM --- Step 4: Start Open-LLM-VTuber ---
echo.
echo [4/4] Starting Open-LLM-VTuber ...
echo.
